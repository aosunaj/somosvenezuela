...
import type { BackendClient, SessionStore, TelegramTransport } from "./ports.js";
...
async function runConversation(
  ...
  await runEffect(chatId, result, deps);
}
...
async function runEffect(
  ...
  if (effect.type === "delete_person") {
    await deps.transport.sendMessage(chatId, DELETE_NOT_AVAILABLE);
 ...
 *
 * NUNCA pide ni procesa `contact_id`: create_person manda lo que arma la maquina
 * (sin contacto) y search_persons recibe la vista publica.
 */
