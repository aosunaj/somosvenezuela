...
// Espejo del poller del bot de Telegram; misma logica, distinta plataforma.
//
// El backend ACUMULA notificaciones pendientes (p. ej. "apareció una coincidencia
// con tu búsqueda"); este poller las RETIRA y las entrega por WhatsApp, marcándolas
// como enviadas solo cuando el envío tiene éxito. Así el backend no necesita hablar
// WhatsApp: cada bot entrega lo suyo.
//
...
//
// GUARDRAILS:
//   - El `chat_id` (wa_id) es dirección de transporte, NO PII a imprimir: nunca se loggea.
...
/** Envío saliente por el bot: entrega un texto a un chat (id como cadena = wa_id). */
 ...
 *   2. Filtra solo las de ESTA plataforma (whatsapp).
...
export async function pollOnce(
  ...
  try {
    ...
    logger.error("[bot-whatsapp] No se pudieron leer notificaciones pendientes.");
  ...
  for (const n of pending) {
    ...
    if (deliveredUnmarked.has(n.id)) {
      try {
        ...
        failed += 1;
        logger.error("[bot-whatsapp] Reintento de marcado de una notificación ya entregada falló.");
      }
      continue;
    }
    ...
    if (text === null) {
      ...
      try {
        ...
        failed += 1;
        logger.error("[bot-whatsapp] Fallo al marcar una notificación sin cuerpo.");
      }
      continue;
    }

    try {
      // El chat_id (wa_id) es transporte; el texto ya viene seguro desde el backend.
      ...
      failed += 1;
      logger.error("[bot-whatsapp] Fallo al entregar una notificación; se reintentará.");
      continue;
    }
    ...
    try {
      ...
      failed += 1;
      logger.error("[bot-whatsapp] Notificación entregada pero no se pudo marcar como enviada.");
    }
  }
 ...
 * para correr en paralelo al servidor webhook de WhatsApp en `index.ts`.
 */
